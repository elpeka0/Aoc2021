...
namespace Aoc.y2021
{
    public class Day23 : DayBase
    {
        private class Spot
        {
            ...
            public bool IsMatch => this.Y > 0 && this.AmphiPod != null && GetScore(this.X) == this.AmphiPod.Value;
            ...
            public override string ToString()
            {
                ...
                {
                    return $"{this.X}, {this.Y}: {this.AmphiPod.Value}";
                }
                else
                {
        ...
        private class Board
        {
            ...
            public int? MovementCost(int fx, int fy, int tx, int ty)
            {
                ...
                {
                    this.Spots.TryGetValue((x, y), out var s);
                    ++steps;
            ...
            {
                for (var cy = y + 1; cy <= this.Depth; ++cy)
                {
                    this.Spots.TryGetValue((x, cy), out var spot);
                    yield return spot;
                }
            }
            ...
            public (int?, List<Move>) Solve(string indent)
            {
                ...
                foreach(var s in this.Spots.Values)
                {
                    if (!s.IsLocked)
                    {
                        if (s.IsMatch && this.Below(s.X, s.Y).All(s => s != null && s.IsMatch))
                        {
                            this.Spots[(s.X, s.Y)] = new Spot(s.X, s.Y, s.AmphiPod, true);
                        }
                        else if(s.Y < 2 || !this.Spots.ContainsKey((s.X, s.Y - 1)))
                        {
                            misplaced.Add(s);
                        }
                    }
                ...
                {
                    var cost = this.MovementCost(s.X, s.Y, x, y);
                    if (cost != null)
                    {
                ...
                foreach (var s in misplaced)
                {
                    ...
                    for (var cy = this.Depth; cy >= 1; --cy)
                    {
                        if (!this.Spots.TryGetValue((x, cy), out var target))
                        {
        ...
        private Board GetInput(int depth)
        {
            ...
            for (var x = 0; x < 4; ++x)
            {
                for (var y = 0; y < 2; ++y)
                {
                    var ax = 2 + 2 * x;
                    var ay = y == 0 ? 1 : depth;
                    var s = new Spot(ax, ay, this.Seed[2 * x + y], false);
                    res.Spots[(ax, ay)] = s;
                }
            }
